...
class ParsedPage(Base):
    """Результат парсинга веб-страницы (заполняется парсером в ЛР2/ЛР3).

    Эта таблица — точка интеграции парсера с БД: и HTTP-парсер, и Celery-задача
    сохраняют сюда заголовки распарсенных страниц.
    """
    ...
    # Каким способом получена запись: http / celery.
